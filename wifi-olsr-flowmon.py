...
def main(argv):
    ...
    cmd.DISTANCE = None
    cmd.AddValue("DISTANCE", "Distance beetween nodes")
    ...
    for xi in range(num_nodes_side):
        for yi in range(num_nodes_side):
            ...
            mobility = ns.mobility.GaussMarkovMobilityModel()
    ...
    def print_stats(os, st):
        ...
        with open("OLSR_FlowStats_%s_%s.txt"%(NUM_NODES_SIDE, DISTANCE),"a") as flowFile:
            flowFile.write("%s %s %s\n"%(str(st.txPackets), str(st.rxPackets), str(st.lostPackets)))
    ...
    if cmd.Results is  None:
        for flow_id, flow_stats in monitor.GetFlowStats():
            ...
            print monitor.SerializeToXmlFile('OLSRGAUSSMARKOV%s_%s'%(NUM_NODES_SIDE,DISTANCE), True, True)
            #with open('dataFlow_%s_%s'%(NUM_NODES_SIDE,DISTANCE), 'a') as dataFlow:
                #dataFlow.write(flow_stats.lostPackets / flow_stats.txPackets)
    else:
        print monitor.SerializeToXmlFile('OLSRGAUSSMARKOV%s_%s'%(NUM_NODES_SIDE,DISTANCE), True, True)
